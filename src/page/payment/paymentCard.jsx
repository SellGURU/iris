...
export const PaymentCard = ({bundle, price, packageId}) => {
    return (
        <>
            <div
                className={"rounded-2xl px-3 py-2 gap-3 shadow-10xl flex items-center justify-center flex-col border-2  w-fit min-w-[303px]"}>
                <h2 className={"text-lg font-medium mt-16"}>package {packageId}</h2>
                <h1 className={"text-3xl font-medium text-[#544BF0]"}>${price} per scan</h1>
                <div className={"flex items-center justify-center gap-4 flex-col"}>
                    <h1 className={"border-b font-medium text-xl w-full text-center mt-8"}>{bundle}</h1>
                ...
                <div
                    ...
                    <ButtonPrimary>Get it now</ButtonPrimary>
                </div>
            </div>
        </>
    )
